function[Out]=MINDy_Linear(X0,dX0,Pre,ParStr,Wmask,slope)  %#ok<INUSL>
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%                  Optional slope=Slope of the linear activation function (default=1)
%%
%% Note: the slope of the MINDy nonlinear activation function at the origin is around 1.3; the best linear
%%       approximation across the range of inputs (standard normal distribution) is around 0.62.
...
if ~isempty(Wmask) && ((mean(size(Wmask)==nX)~=1)||~isempty(find(~ismember(Wmask(:),[0 1]),1)))
    error('Wmask should be n x n binary or empty/unspecified[default: all connections]')
end
A = (slope .^ 2 + 0.25) .* (B1(:, 1) .^ 2);  % In case of error in |ModularRecord|
...
for iBatch=1:NBatch
    ...
    Decay=Dmin+D.^2;  % Dmin is specified in ChosenPARSTR with default of 0.1
    WK=Wk1*Wk2;       % Low rank component of connectivity matrix W0
    W0=W1+WK;         % W1 is the sparse component of W0
    ...
    % The first term (P1) of the activation can be written as b * sqrt[(a^2 + 0.25)/(b^2) + x(x + 1/b)]

    % bbTemp=X+B1(:,2); %#ok<NODEF> B1(:, 1) = 1/b = 3/20, B1(:, 2) = c (shift of x)

    % P1=sqrt(A+(bbTemp.*(bbTemp+B1(:,1))));  % A = (a^2 + 0.25) / (b^2)
    ...
    end  % E: prediction error (not squared)

    gradD=-2*D.*mean(E.*X,2);  % (negative, same below) gradient for D
    gradW1=(E*PolyZ')/BatchSz;  % It's actually the gradient for W0
    ...
    if L2SpPlsEN~=0
        WcTemp=gradW1-L2SpPlsEN*WK;  % Gradient for Wk (low rank part) for E + L2norm
    ...
    wCost=((SpScale)*sign(W1)+(2*ENL2)*W1);  % Just the derivative of L1 norm (ENL2 was set to 0)
    gradW1=gradW1-wCost-diag(SpDiag*sign(diag(W1)));  % Gradient for W1 (sparse part)

    % W1E=W0'*E;  % Gradient of PolyZ
    % DpTemp=(B1(:,1).^-1).*W1E.*((1./P1)-(1./P2));  % 2 * gradient of A

    %% Really fitting for (A.^-.5)
    % gradA=-A.^(1.5).*mean(DpTemp,2);  % The gradient for A^(-1/2) = b / sqrt(a^2 + 0.25) = Xi (mentioned above)
